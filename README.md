# Initial test
# From calculation of throughput, we have about a rate of 75-80 in total for 5 keys, so about 15-16 requests per second out of a possible 20
# No request rate overflow at all


# Assumption / Question 1
# While we are trying to optimise throughput only, how much priority do we give to time between order creation or order sent?
...
# in we forgo the limit of 1 sec between order creation and order sending, we can increase the throughput, although we might not want that

# ISSUES: 1
# There is some overhead in each call such that in acquire, which adds to the time between orders
# Thus, when we have n time between calls, where n = 1 sec / max_calls_per_sec, instead we are having n + x time, where x is the overhead
...
# lower sleep time -> less time between (a) actual time we can make request (according to rate limit) and (b) time when granted access
# higher sleep time -> less overhead since async process is sleeping for longer
...
# - Ignored from queue -> Time between order and processing
    # Maybe if time remaining is too little, do not pass to Limiter since its unlikely to process
# - Limited by Limiter -> Time between order and request
# - Exceeds -> since the accumulate, might need to keep it to 0, IRL: probably resets
